...
"""
lp4/build.py
マスターHTML（lp4/index.html）をベースに、各ASPディレクトリのindex.htmlを生成するスクリプト。
JSONの内容をビルド時に直接<head>に焼き込むため、実行時のJSONフェッチが不要。

使い方:
  cd /home/ubuntu/ametome-lp-asp1/lp4
  python3 build.py

マスター（lp4/index.html）を修正したら、このスクリプトを実行して
...
def build_tel_mode_script(config):
    ...
    tel_raw = config.get('tel_raw', '05055745586')
...
def main():
    ...
    print("  git add lp4/ && git commit -m 'build: ASP別HTMLを再生成' && git push origin master")
