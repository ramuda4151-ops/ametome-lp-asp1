...
"""
lp3/build.py
マスターHTML（lp3/index.html）をベースに、各ASPディレクトリのindex.htmlを生成するスクリプト。
JSONの内容をビルド時に直接<head>に焼き込むため、実行時のJSONフェッチが不要。

使い方:
  cd /home/ubuntu/ametome-lp-asp1/lp3
  python3 build.py

マスター（lp3/index.html）を修正したら、このスクリプトを実行して
...
def main():
    ...
    print("  git add lp3/ && git commit -m 'build: ASP別HTMLを再生成' && git push origin master")
